    ...
    for s=1:S
        ...
        for t=1:Tmax
            ...
            if SNcomTS(y,s,t)>0 % if SN is assembled
                ...
                TimeAreaProp(y,s,t)=numel(u)/E(y); %Bug update (E)
...
TimeAreaProp(isnan(TimeAreaProp))=0; % Bug update E
    ...
    for i=1:C
        ...
        if size(Temp,3)>1 %more than one SN
            for t=1:Tmax
                ...
                bb=unique(nonzeros(Temp(:,t,:))); % unique areas at time t
                if bb>0
                    AA(bb)=AA(bb)+1;
                end
            end
...
if S>1
    for i=1:C
        MNMeanArea(i,:)=mean(TimeAreaMetaW(:,i,:));%relative to all time
        MNMeanInt(i,:)=mean(TimeAreaMetaInt(:,i,:));
    end
else
    MNMeanArea=squeeze(TimeAreaMetaW(1,:,:));%relative to all time
    MNMeanInt=squeeze(TimeAreaMetaInt(1,:,:));
end

save MetaProbMaps MNMeanArea MNMeanInt TimeAreaMetaInt TimeAreaMetaW MeanQintMNsubj %MeanMetaQint%Int is relative to networks own time of assembly
...
for i=1:C
    ...
    r=find(HierOrd(:,2)==i)
    aa=numel(unique(nonzeros(SNs(HierOrd(r,1),:)))); % Bug update
...
for i = 1:C
    ...
    netsWe=MNMeanInt(i,nonzeros(MNarea(i,:))); %the weighting of the areas in the MN
...
%example one subject
MNstate=squeeze(StateNonZeroSorted(:,1,:));
...
example=squeeze(StateNonZeroSorted(:,1,:));
figure; hold on
t=10
