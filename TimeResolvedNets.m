...
% (Row 62-116 and 281-309 are adapted from Cabral et al 2017, Leida weights are
% calculated but not used in the paper)
...
% 62,68,1334,1344,1568,1577,1622,1631 needs to be utdated to fit the
% local environement.
...
%division as well as Leida weights
    ...
    save(sprintf('Louvain%d_%d',j,K), 'Leida_IMF_m', 'Louvain_s',  '-v7.3')
...
save QRsTOP_IMFiter QRtopS Q QM
figure; imagesc(QM)
...
for w=1:length(D(:,1,1))
    ...
    parfor k=1:L
	    ...
	    if SNC_size==8 
            ...
            temp4=squeeze(Leida(s,:,:));
            temp5=squeeze(Phase(s,:,:));
            temp4(~logical(zzz),:)=0;
            
            SNCmeanLeida(s,k,:)=mean(temp4(:,snc),2);% mean Leida value of the SNC at each ts
    ...
    save(sprintf('SNC8_TS_%d',w),'Qint_SNC', 'SNCbinTS','SNCcomTS','SNCmeanPhase','SNCmeanLeida','-v7.3')    
end
...
T2=zeros(Y,S,Tmax);
Bb=zeros(Y,S,3);
SNmeanLeida=zeros(Y,S,Tmax);
SNmeanPhase=zeros(Y,S,Tmax);

for y=1:Y
   y 
    clear Qint_SNC SNCbinTS SNCcomTS SNCmeanLeida SNCmeanPhase
        ...
        for t=1:Tmax
            ...
            if a>0 %if any SNC belonging to the SN is integrated
                if numel(a)==1 % if all in the same community
                    ...
                    SNmeanLeida(y,s,t)=mean(nonzeros(SNCmeanLeida(s,:,t)));
                ...
                elseif numel(a)>1 %if split into more than one community
                    ...
                    if numel(a)==2
                        ...
                        if a1>a2
                            ...
                            T2(y,s,t)=a2/(a1+a2);
                            SNmeanLeida(y,s,t)=mean(nonzeros(SNCmeanLeida(s,r,t)));
...
save SN_Time T2 CommunitySplit QintSN MeanQintSN SNmeanLeida SNmeanPhase SNcomTS ww Bb
...
save(sprintf('MNQint_%d',C), 'MN_TS', 'QintMNsubj', 'MeanQintMN')
...
[MeanMetaQintSorted, Isort] =sortrows(MeanQintMN,1,'descend');
...
save SortedHierOrd  Hsort MeanMetaQintSorted Isort
...
for s=1:S
    ...
    for i=1:C
        ...
        Mean_MetaQ(s,i)=sum(B)/Tmax; %relative freq of MN in relation to all time
...
if S>1
    for i=1:C
        MetaMean(i,:)=mean(TimeAreaMetaW(:,i,:));%relative to all time
        MetaMeanInt(i,:)=mean(TimeAreaMetaInt(:,i,:));
    end
else
    MetaMean=squeeze(TimeAreaMetaW(1,:,:));%relative to all time
    MetaMeanInt=squeeze(TimeAreaMetaInt(1,:,:));
end

save MetaProbMaps MetaMean MetaMeanInt TimeAreaMetaInt TimeAreaMetaW Mean_MetaQ %MeanMetaQint%Int is relative to networks own time of assembly
...
for i=1:length(MNmap(:,1))
    ...
    title(['MN ', num2str(i), ', Qint ',num2str(round(MeanMetaQintSorted(i,1),2))], 'FontSize',30, 'FontWeight', 'bold')
...
for i = 1:C
    ...
    elseif numel(re)==2
        MN_Net_Name(i,1:2)=netu(re);% Number of largest contributor
        w2=setdiff(w,max(w)); %identify second largest weight
        if  max(w2)/max(w)>1/3
            re2=find(w==max(w2));
            if numel(re2)==1
                MN_Net_Name(i,3)=netu(re2);
            else % in small networks weights can be whole numer and equal for more than one netw
                MN_Net_Name(i,3:4)=netu(re2);
            end
            ...
            if numel(re2)==1 % Add more only if there is only 2 names before
                ...
                if max(w3)/max(w)>1/3
                    re3=find(w==max(w3))
                    if numel(re3)==1
                        MN_Net_Name(i,4)=netu(re3);
                    else
                        MN_Net_Name(i,4:5)=netu(re3);
                    end
                end
            else
            end
        ...
        MN_Net_Name(i,1:3)=netu(re);
        
    end
end

MNclassSorted=MNclass(Isort);
MN_Net_NameSorted=MN_Net_Name(Isort,:);

for i=1:C
    temp=MN_Net_NameSorted(i,:)
   a= numel(nonzeros(temp))
   for k=1:a
       MNnames(i,k)=NetName(temp(k),2)
   end
end


save MNnaming MNclass  MN_Net_Name MN_Net_NameSorted MNclassSorted
